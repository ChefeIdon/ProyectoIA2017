...
%Tirar
action(drop(Obj), [has([agent,me],Obj), at([agent,me],MiPos)], [at(Obj,MiPos)], [has([agent,me], Obj)]).
...
action(
    depositarTesoro(Tesoro), %Depositar un tesoro

    [has([agent,me],[gold,Tesoro]),property([agent,me],home,MiCasa),at([home,MiCasa],PosCasa),at([agent,me],PosCasa)],
    %Precondiciones: Tengo tesoro, estoy en mi casa
