...
checkearHas(Due駉, Objeto, Percepcion):-
	%write('Estoy viendo a '),write(Due駉),write(' con '),write(Objeto),write(' en su poder?'),
	member(has(Due駉,Objeto), Percepcion),
	%writeln(' SI!'),
	!.
checkearHas(Due駉, Objeto, _Percepcion):-
	retract(has(Due駉,Objeto)).
