...
display_ag:-
	...
	nl,
	ag_name(AgName),
	write('Agente: '), write(AgName), write('      '),

	time(Time),
	write('Tiempo: '), write(Time), write('	   '),

        property([agent, me], life, AgStamina),
	property([agent, me], lifeTotal, AgMaxStamina),
        write('vida: '), write(AgStamina), write(' / '), write(AgMaxStamina), write('	   '),

	property([agent, me], skill, _AgSkill),
...
display_entity(Entity):-
	at(Entity, Pos),
	atPos(Entity, Vector),
	write('Recuerdo a '), write(Entity), write(' en el nodo '), write(Pos), write(' ('), write(Vector), write(')'), write('.'),
	implies(Entity = [agent, _], nl),
	entity_descr(Entity, Descr),
	write('  Descripcion: '), write(Descr), write('.'),
	implies(has(Entity, _), (implies(Entity = [agent, _], nl), write('  Tiene: '))),
...
display_entity(Entity):-
	has(_, Entity),
	entity_descr(Entity, Descr),
	write(' '), write(Entity), write(' descripcion: '), writeln(Descr).
